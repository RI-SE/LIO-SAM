...
class IMUPreintegration : public ParamServer {
   public:
	...
	IMUPreintegration() {
		...
		noiseModelBetweenBias = (gtsam::Vector(6) << imuAccBiasN, imuAccBiasN, imuAccBiasN, imuGyrBiasN,
								 imuGyrBiasN, imuGyrBiasN)
									.finished();
		// noiseModelBetweenBias = (gtsam::Vector(6) << -0.0003, -0.0004, 0.0001, -0.06818197762056796,
		// 0.09482451509415125, 0.07906817572805361).finished();
	...
	void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg) {
		...
		while (!imuQueOpt.empty()) {
			...
			if (imuTime < currentCorrectionTime - delta_t) {
				...
				if (dt > 2 * (1.0 / imuFrequency)) {
					std::cout << "integration time is high #1! "
							  << "\ndt: " << dt << " imuFrequency: " << imuFrequency
							  << " 2*(1/imuFrequency: " << 2 * (1.0 / imuFrequency) << std::endl;
				}
		...
		if (!imuQueImu.empty()) {
			...
			for (int i = 0; i < (int)imuQueImu.size(); ++i) {
				...
				if (dt > 2 * (1.0 / imuFrequency)) {
					std::cout << "integration time is high #2! "
							  << "\ndt: " << dt << " imuFrequency: " << imuFrequency
							  << " 2*(1/imuFrequency: " << 2 * (1.0 / imuFrequency) << std::endl;
				}
	...
	void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw) {
		...
		if (dt > 2 * (1.0 / imuFrequency)) {
			std::cout << "integration time is high #3! "
					  << "\ndt: " << dt << " imuFrequency: " << imuFrequency
					  << " 2*(1/imuFrequency: " << 2 * (1.0 / imuFrequency) << std::endl;
		}
