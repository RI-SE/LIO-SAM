...
POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZIRPYT,
								  (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)(
									  float,
									  roll,
									  roll)(float, pitch, pitch)(float, yaw, yaw)(double, time, time)(float,
																									  rgb,
																									  rgb))
...
class mapOptimization : public ParamServer {
   public:
	...
	mapOptimization() {
		...
		pubIcpKeyFrames
			= nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/icp_loop_closure_corrected_cloud", 1);
	...
	void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn) {
		// std::cout << "Got Cloud Info to MapOptimization" << std::endl;
		...
		// std::cout << "Extracted feature clouds in MapOptimization" << std::endl;
		...
		if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval) {
			...
			// std::cout << "updated timeLastProcessing to " << timeLastProcessing << std::endl;

			updateInitialGuess();
			// std::cout << "passed updateInitialGuess" << std::endl;

			extractSurroundingKeyFrames();
			// std::cout << "passed extractSurroundingKeyFrames" << std::endl;

			downsampleCurrentScan();
			// std::cout << "passed downsampleCurrentScan" << std::endl;

			scan2MapOptimization();
			// std::cout << "passed scan2MapOptimization" << std::endl;

			saveKeyFramesAndFactor();
			// std::cout << "passed saveKeyFramesAndFactor" << std::endl;

			correctPoses();
			// std::cout << "passed correctPoses" << std::endl;

			publishOdometry();
			// std::cout << "passed publishOdometry" << std::endl;

			publishFrames();
			// std::cout << "passed publishFrames" << std::endl;
		}
	}

	void gpsHandler(const nav_msgs::Odometry::ConstPtr& gpsMsg) {
		std::scoped_lock tmpmtx(mtxGpsQueue);
		// std::cout << "Got GPS to MapOptimization" << std::endl;
...
#pragma omp parallel for num_threads(numberOfCores)
	...
	bool saveMapService(lio_sam::save_mapRequest& req, lio_sam::save_mapResponse& res) {
		...
		*globalMapCloud += *globalSurfCloud;
		...
		try {
			...
			ofs << boost::format("%.6f %.6f %.6f") % utmToMapTransform.getOrigin().x()
					   % utmToMapTransform.getOrigin().y() % utmToMapTransform.getOrigin().z()
				<< std::endl;
			ofs.close();

			// Transform global map
			utmToMapQuat = utmToMapTransform.getRotation();
			alignMapToNorth.rotate(Eigen::AngleAxisf(
				utmToMapQuat.getAngle(),
				Eigen::Vector3f(utmToMapQuat.getAxis().getX(), utmToMapQuat.getAxis().getY(),
								utmToMapQuat.getAxis().getZ())));
		...
		*copy_cloudKeyPoses6D = *cloudKeyPoses6D;
		...
		pcl::PointCloud<PointType>::Ptr prevKeyframeCloud(new pcl::PointCloud<PointType>());
		{
			...
			if (cureKeyframeCloud->size() < 300 || prevKeyframeCloud->size() < 1000) {
				// std::cout << "cureKeyframeCloud->size() < 300 || prevKeyframeCloud->size() < 1000" <<
				// std::endl;
				return;
			}
		...
		if (icp.hasConverged() == false || icp.getFitnessScore() > historyKeyframeFitnessScore) {
			// std::cout << "icp.hasConverged() == false || icp.getFitnessScore() >
	...
	void updateInitialGuess() {
		...
		// auto closesttimestamp = std::lower_bound(gpsQueue.begin(), gpsQueue.end(), timeLaserInfoCur,
		// compareFunctor); if(closesttimestamp == gpsQueue.end()) {
		//     closesttimestamp--;
		//     if(timeLaserInfoCur-closesttimestamp->header.stamp.toSec() > 0.2) {
		//         std::cout << "Missing GPS message within time barrier" << std::endl;
		//         std::cout << "Current Scan time: " << std::setprecision(15) << timeLaserInfoCur <<
		//         std::endl; std::cout << "Last GPS time: " << std::setprecision(15) <<
		//         closesttimestamp->header.stamp.toSec() << std::endl; std::cout << "Message is: " <<
		//         timeLaserInfoCur-closesttimestamp->header.stamp.toSec() << " seconds old" << std::endl;
...
#pragma omp parallel for num_threads(numberOfCores)
	...
	bool LMOptimization(int iterCount) {
		...
		// transformation lidar <- camera      ---     camera <- lidar x = z                ---     x = y y =
		// x                ---     y = z z = y                ---     z = x roll = yaw           ---     roll
		// = pitch pitch = roll         ---     pitch = yaw yaw = pitch          ---     yaw = roll
		...
		float deltaR
			= sqrt(pow(pcl::rad2deg(matX.at<float>(0, 0)), 2) + pow(pcl::rad2deg(matX.at<float>(1, 0)), 2)
	...
	void publishOdometry() {
		...
		// tf::StampedTransform trans_odom_to_lidar = tf::StampedTransform(t_odom_to_lidar,
		// timeLaserInfoStamp, odometryFrame, "lidar_link"); br.sendTransform(trans_odom_to_lidar);
			...
			if (lastSLAMInfoPubSize != cloudKeyPoses6D->size()) {
				...
				*cloudOut += *laserCloudSurfLastDS;
				...
				slamInfo.key_frame_poses
					= publishCloud(ros::Publisher(), cloudKeyPoses6D, timeLaserInfoStamp, odometryFrame);
